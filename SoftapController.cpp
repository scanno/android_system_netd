...
static int ensure_config_file_exists()
{
    ...
    if (access(HOSTAPD_CONF_FILE, R_OK|W_OK) == 0) {
        ...
        ALOGE("Cannot access \"%s\": %s", HOSTAPD_CONF_FILE, strerror(errno));
        return -1;
    }
    ...
    if (srcfd < 0) {
        ALOGE("Cannot open \"%s\": %s", HOSTAPD_CONF_TEMPLATE, strerror(errno));
        return -1;
    }
    ...
    if (destfd < 0) {
        close(srcfd);
        ALOGE("Cannot create \"%s\": %s", HOSTAPD_CONF_FILE, strerror(errno));
        return -1;
    }

    while ((nread = read(srcfd, buf, sizeof(buf))) != 0) {
        if (nread < 0) {
            ALOGE("Error reading \"%s\": %s", HOSTAPD_CONF_TEMPLATE, strerror(errno));
            close(srcfd);
            close(destfd);
...
static int wifi_start_hostapd()
{
    ...
    if (ensure_config_file_exists() < 0) {
        ALOGE("Wi-Fi HostAP will not be enabled");
        return -1;
    }
...
static int wifi_connect_to_hostapd()
{
    ...
    if (!property_get(HOSTAPD_PROP_NAME, supp_status, NULL)
            || strcmp(supp_status, "running") != 0) {
        ALOGE("hostapd not running, cannot connect");
        return -1;
    }
...
static int wpa_ctrl_command_sta(const char *cmd, char *addr, size_t addr_len)
{
	...
	if (ctrl_conn == NULL) {
		ALOGE("Not connected to hostapd - command dropped.\n");
		return -1;
	}
	len = sizeof(buf) - 1;
...
int SoftapController::stopDriver(char *iface) {
    ...
    *mBuf = 0;
    ...
    ifc_close();
    if (ret < 0) {
        ALOGE("Softap %s down: %d", AR6002AP_IFNAME, ret);
    }
...
int SoftapController::startSoftap() {
	...
	if (ret < 0) {
		ALOGE("Softap startup - configuration file missing");
...
int SoftapController::fwReloadSoftap(int argc, char *argv[])
{
	...
	if (ret) {
		ALOGE("Softap fwReload - Firmware not loaded!");
	}
